...
def Holstein2_rev(q, params, full_id):
    critical_params = ["E_n", "x_n", "k_n"]
    default_params = {"V": 0.001}
    comn.check_input(params, default_params, critical_params)

    E_n = jnp.array(params["E_n"], dtype=jnp.float64)
    x_n = jnp.array(params["x_n"], dtype=jnp.float64)
    k_n = jnp.array(params["k_n"], dtype=jnp.float64)
    V   = float(params["V"])
    n   = E_n.shape[0]
    ...
    def H_real(x_scalar, E_n, x_n, k_n, V):
        ...
        H = H.at[jnp.arange(n), jnp.arange(n)].set(diag)
        idx = jnp.arange(n - 1)
        H = H.at[idx, idx + 1].set(V)
    ...
    # Promote to complex for your CMATRIX
    H_val = H_real_val.astype(jnp.complex128)
    dHdx = dHdx_real.astype(jnp.complex128)
    ...
    for i in range(n):
        for j in range(n):
            Hdia.set(i, j, complex(H_val[i, j]))
            d1ham_dia[0].set(i, j, complex(dHdx[i, j]))
...
def Holstein2_grad(q, params, full_id):
    ...
    # Extract scalar nuclear coordinate x
    ...
    def Hii(x, i):
        return (E_n[i] + 0.5 * k_n[i] * (x - x_n[i])**2)

    dHii_dx = jax.grad(Hii, argnums=0)
    ...
    for i in range(n):
        d1ham_dia[0].set(i, i, complex(dHii_dx(x_jax, i)))
